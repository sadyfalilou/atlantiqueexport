-- Rattache une commande au compte qui la passe.
--
-- `place_order` ne renseignait jamais `orders.user_id`. Or la politique
-- `orders_select_own` filtre précisément là-dessus : un client connecté ne
-- voyait donc AUCUNE de ses commandes dans son espace. Le défaut est resté
-- invisible tant que l'espace client n'existait pas.
--
-- Cette migration reprend la fonction d'origine **mot pour mot** et n'y change
-- que deux choses : le paramètre, et la colonne dans l'insertion. Réécrire de
-- mémoire la transaction la plus critique du système — réservation de stock,
-- prise de créneau, calcul des montants — aurait été le meilleur moyen d'y
-- glisser une régression silencieuse.
--
-- Le paramètre est ajouté en FIN de signature avec une valeur par défaut : les
-- appels existants continuent de fonctionner, et une commande sans compte
-- garde `user_id` à NULL, consultable par son jeton comme avant.
